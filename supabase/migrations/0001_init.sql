...
create policy "members can read their crew"
  on public.crews for select using (
    exists (
...
create policy "members can read crew logs"
  on public.pushup_logs for select using (
    crew_id in (select crew_id from public.crew_members where user_id = auth.uid())
  );

create policy "users can insert their own logs"
  on public.pushup_logs for insert with check (user_id = auth.uid());
...
-- ─── REALTIME ────────────────────────────────────────────────────────────
-- Enable realtime broadcasts for logs + chat (Supabase Dashboard does this
-- via toggles, but you can also do it in SQL).
alter publication supabase_realtime add table public.pushup_logs;
alter publication supabase_realtime add table public.chat_messages;

-- ─── HELPER VIEWS ────────────────────────────────────────────────────────
-- Today / week / lifetime counters per user (used by the app instead of
-- recomputing in JS — RLS rules above still apply through the underlying tables).
